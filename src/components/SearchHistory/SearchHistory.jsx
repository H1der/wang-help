  ...
  return (
    <View>
      ...
      <View className='history-info'>
        {
          history.length > 0 ? history.map((item, index) => {
            return (<AtTag className='history-tag' key={index} circle color='grey' onClick={()=>{getSearchKeyword(item)}}>{item}</AtTag>)
          }) : ''
      ...
      <AtModal isOpened={isOpened}>
        ...
        <AtModalAction> <Button onClick={()=>setIsOpened(false)}>取消</Button> <Button onClick={handleConfirm}>确定</Button> </AtModalAction>
      </AtModal>
    </View>
  )
}
